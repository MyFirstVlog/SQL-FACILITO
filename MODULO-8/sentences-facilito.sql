...
--SELECT * FROM libros WHERE titulo LIKE 'Harry Potter%';  (El porcentaje aparece en la parte derecha porque se quiere obtener todos los registros que empiecen con los strings Harry Potter)


--SELECT * FROM libros WHERE titulo LIKE '%anillo';   (El porcentaje aparece en la parte izquierda porque se quiere obtener todos los registros que terminen con el string anillo);


--SELECT * FROM libros WHERE titulo LIKE '%Harry%'; saca los mismos resultados que 
...
-- --> Libros cuyo titulo empiece con H o con L

-- Para evitar muchos usos LIKE concatenados o con LEFT y RIGHT

-- SELECT titlo FROM libros WHERE titulo REGEXP '^[HL]';
...
-- podemos paginar resultados con limit 

-- → SELECT libro_id, titulo FROM libros LIMIT 0, 5; ( 0 → desde cual, 5 → Cantidad de registros)


-- DELIMITER % 

-- CREATE FUNCTION getSells()
-- RETURNS INT
-- BEGIN
--   RETURN (SELECT (ROUND(RAND() * 100) * 6)) ;
-- END%

-- DELIMITER ;

-- UPDATE libros SET ventas = getSells();
...
-- SELECT autor_id, SUM(ventas) FROM autores GROUP BY autor_id ORDER BY DESC LIMIT 1;
...
-- Si queremos hacer consulta anidad
...
-- Ahora Para consultar el nombre completo, otra subconsulta
