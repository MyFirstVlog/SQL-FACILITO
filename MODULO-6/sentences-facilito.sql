...
-- ('Ojos de fuego', 'Cujo', 'El hobbit', 'La torre oscura 7 La torre oscura')

-- SELECT * FROM libros WHERE titulo IN ('Ojos de fuego', 'Cujo', 'El hobbit', 'La torre oscura 7 La torre oscura');

-- SELECT * FROM books WHERE titulo = 'Carrie' ;

-- SELECT * FROM books WHERE (autor_id = 1 AND titulo = ‘Carrie’) OR (autor_id = 5 AND titulo = ‘El hobbit’)
-- SELECT * FROM autores WHERE seudonimo IS NOT NULL;
-- SELECT * FROM autores WHERE seudonimo <=> NOT NULL;
-- SELECT titulo FROM libros WHERE fecha_publicacion BETWEEN '2000-01-01' AND '2020-12-31'; 
-- SELECT DISTINCT titulo FROM libros;
-- SELECT DISTINCT titulo AS libro FROM libros;
-- SELECT 9/3 AS resultado;


-- → UPDATE libros SET descripcion = 'Nuevo cambio', ventas = 50; (cuidado si no se le pone where actualiza todos los registros con los atributos del set, para filtrar la cantidad de registros debemos de usar WHERE)

-- → UPDATE libros SET descripcion = 'Nuevo cambio', ventas = 50 WHERE titulo = “El hobbit” ; 


-- → DELETE FROM libros; (eliminaría todos los registros de la tabla pues no específica ningún condicional)

-- → DELETE FROM libros WHERE author_id = 1;
-- → DELETE FROM libros WHERE libro_id = 55;

-- Se deben eliminar los registros que utilice su llave primaria. 
...
-- → TRUNCATE TABLE libros (no puede usar where, registros y metadatos se eliminan); == DELETE FROM libros; (aquí los metadatos se conservan entonces los ids los tiene en cuenta por lo que si se agrega un registro nuevo aunque se haya eliminado la tabla con 20 registros un nuevo insert hará que empiece la tabla por el id 21, en cambio con truncate si empezaria con un 1)
